...
// Persistence for the UPI verifier: pending ORDERS (a plan someone intends to
// pay for, tagged with a unique amount) and RECEIPTS (parsed bank credit
// alerts). Both live in one Cosmos container discriminated by `type`, with an
// in-memory Map fallback when no connection string is set (local dev / tests).
//
// @azure/cosmos is required lazily so this module can be imported by unit tests
// without the dependency loaded.

const { sameAmount } = require('./parse');

const CONN = process.env.COSMOS_CONNECTION_STRING || '';
const DB_NAME = 'upiverifier';
const CONTAINER_NAME = 'records';
...
// All currently-pending orders (not yet confirmed, not expired). Used to pick a
// free unique amount and to match an incoming receipt.
...
async function getOrder(id) {
  ...
  // Lazily expire a stale pending order so status reads reflect reality.
...
// match it to a pending order by exact amount. Returns { receipt, order? }.
async function ingestCredit({ amount, utr, rawText, source }) {
  ...
  // confirm two orders. If we've already recorded this UTR, no-op the match.
  ...
  // Match the MOST RECENT pending order of this amount — the one the payer is
  // most likely settling now. Older unpaid orders stay pending and expire.
